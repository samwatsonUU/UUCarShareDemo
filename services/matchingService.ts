  ...
  */
  const results = await db.getAllAsync<Journey & { firstName: string }>(
    ...
    [
      ...
      // Compare selected journey must arrive at time to candiate journeys
      selectedJourney.mustArriveAt,
      selectedJourney.mustArriveAt,
      selectedJourney.mustArriveAt,
      selectedJourney.mustArriveAt,
  ...
  const enriched = results
    ...
    .filter(
      (j) =>
        ...
        j.originDistance <= RADIUS_KM && j.destinationDistance <= RADIUS_KM,
    )
    .sort(
      (a, b) =>
