...
type Request = {
  ...
  journeyDate: string;
  journeyDepartingAt: string;
  journeyMustArriveAt: string;
};
...
export default function RequestResponse() {
  ...
  const deny = async () => {
    // Require a non-empty response message before denying
    if (message.trim() === "") {
      Alert.alert("Error", "Add a message.");
      return;
    }
    ...
    const loadData = async () => {
      try {
        ...
        const selectedRequest = await db.getFirstAsync<Request>(
          "SELECT r.requestID, r.message, requester.firstName AS requesterFirstName, requestedJourney.origin AS journeyOrigin, requestedJourney.destination AS journeyDestination, requestedJourney.date AS journeyDate, requestedJourney.departingAt AS journeyDepartingAt, requestedJourney.mustArriveAt AS journeyMustArriveAt FROM requests r JOIN users requester ON requester.userID = r.requesterID JOIN journeys requestedJourney ON requestedJourney.journeyID = r.journeyID WHERE r.requestID = ?",
          [numericRequestID],
        );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.subtitle}>
        Details of the request you are responding to:
      </Text>

      {request && (
        <View style={styles.journeyCard}>
          <Text>Requester: {request.requesterFirstName}</Text>
          <Text>Origin: {request.journeyOrigin}</Text>
          <Text>Destination: {request.journeyDestination}</Text>
          <Text>Departing At: {request.journeyDepartingAt}</Text>
          <Text>Must Arrive At: {request.journeyMustArriveAt}</Text>
          <Text>Date: {request.journeyDate}</Text>
          <Text>Request Message: {request.message}</Text>
        </View>
      )}

      <View style={styles.messageContainer}>
        <Text style={styles.messageLabel}>Optional response message:</Text>
      ...
      <Pressable
        style={({ pressed }) => [
          ...
          pressed && { backgroundColor: "rgba(14, 237, 92, 0.97)" },
        ]}
        onPress={approve}
      >
        {({ pressed }) => (
